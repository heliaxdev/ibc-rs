...
impl KeyEntry {
    pub fn from_key_file(key_file: KeyFile, hd_path: &HDPath) -> Result<Self, Error> {
        ...
        let derived_pubkey_bytes = derived_pubkey.public_key.serialize();
...
pub fn sign_message(
    ...
    let private_key_bytes = key.private_key.private_key.secret_bytes();
    match address_type {
...
fn get_address(pk: ExtendedPubKey, at: &AddressType) -> Vec<u8> {
    match at {
        ...
        AddressType::Cosmos | AddressType::Ethermint { .. } => {
            ...
            hasher.update(pk.public_key.serialize().as_slice());
